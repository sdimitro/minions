...
__global__ void warpDivergenceKernel(float *input, float *output, int n) {
    ...
    if (idx < n) {
        ...
        if (threadIdx.x % 2 == 0) {
            ...
            for (int i = 0; i < 500; i++) {
                result = sqrtf(fabsf(result)) + logf(fabsf(result) + 1.0f);
            }
        }
...
int main() {
    ...
    printf("2. GPU Compute-Intensive Kernel - Block Size Effects\n");
    ...
    for (int i = 0; i < numBlockSizes; i++) {
        ...
        double gpu_time = benchmarkGPU(computeIntensiveKernel, d_input, d_output, n, blockSize);
        
        bool isMultipleOf32 = (blockSize % 32 == 0);
        int wastedThreads = isMultipleOf32 ? 0 : (32 - (blockSize % 32));
        
        printf("   Block size: %3d, Grid size: %4d, Time: %.4f s, Speedup: %5.1fx %s\n", 
    ...
    // Compare regular vs divergent kernels
    double regular_time = benchmarkGPU(computeIntensiveKernel, d_input, d_output, n, 256);
    double divergent_time = benchmarkGPU(warpDivergenceKernel, d_input, d_output, n, 256);
    
    printf("   Regular kernel (256 threads):    %.4f s\n", regular_time);
    printf("   Divergent kernel (256 threads):  %.4f s\n", divergent_time);
    printf("   Divergence overhead: %.1f%%\n\n", ((divergent_time - regular_time) / regular_time) * 100.0);
    
    // Memory bandwidth analysis
    printf("4. Memory Bandwidth Analysis\n");
    
    // Simple memory-bound operation (just copy)
    printf("   Testing memory-bound operation (simple copy)...\n");
    
    // Create a simple copy kernel
    ...
    double copy_time = benchmarkGPU(copyKernel, d_input, d_output, n, 256);
    
    // Calculate effective bandwidth
    double bytes_transferred = 2.0 * bytes; // Read input, write output
    double bandwidth = bytes_transferred / (copy_time * 1e9); // GB/s
    
    printf("   Copy time: %.6f s\n", copy_time);
    printf("   Effective bandwidth: %.2f GB/s\n", bandwidth);
    printf("   Compute-to-memory ratio: %.1fx\n\n", regular_time / copy_time);
    
    // Show why vector addition isn't good for GPU
    ...
    printf("   ✓ Compute-bound operations (like our intensive kernel: %.1fx speedup)\n\n", 
           cpu_time / regular_time);
