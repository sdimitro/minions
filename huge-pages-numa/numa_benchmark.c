...
int main() {
    ...
    // --- Benchmark 2: Remote Huge Page Access ---
    printf("--- Testing Remote Access (CPU on Node 0, Memory on Node 1) ---\n");
    struct bitmask *nodemask_remote = numa_allocate_nodemask();
    numa_bitmask_setbit(nodemask_remote, 1); // Set policy to bind to Node 1

    if (set_mempolicy(MPOL_BIND, nodemask_remote->maskp, nodemask_remote->size + 1)!= 0) {
        perror("set_mempolicy for remote failed");
        numa_free_nodemask(nodemask_remote);
        return 1;
    }
    ...
    buffer = mmap(NULL, BUFFER_SIZE, PROT_READ | PROT_WRITE,
                  MAP_PRIVATE | MAP_ANONYMOUS | MAP_HUGETLB, -1, 0);
    if (buffer == MAP_FAILED) {
        perror("mmap failed for remote huge pages. Ensure enough huge pages are free on Node 1");
        numa_free_nodemask(nodemask_remote);
        return 1;
    }
    memset(buffer, 0, BUFFER_SIZE);
    ...
    time_remote = (end.tv_sec - start.tv_sec) + (end.tv_nsec - start.tv_nsec) / 1e9;
    printf("Time taken (remote access): %.4f seconds\n\n", time_remote);
    ...
    if (time_remote > time_local) {
        double slowdown = ((time_remote - time_local) / time_local) * 100.0;
        printf("Remote access was %.2f%% slower than local access.\n", slowdown);
    } else {
        printf("No significant NUMA effect detected in this run.\n");
    }
